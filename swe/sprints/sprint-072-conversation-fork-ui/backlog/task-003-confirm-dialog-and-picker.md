...
- [ ] The confirm step always displays the exact text of the entry that will be forked from.
- [ ] Confirming issues `fork(entryId)` exactly once even if the confirm control is activated
      repeatedly while pending; buttons are disabled and a spinner shows while in flight.
- [ ] The picker lists the active branch's user messages chronologically with `#N` ordinals.
- [ ] An empty picker shows the disabled "Nothing to fork yet" state rather than an empty dialog.
- [ ] Selecting a picker row swaps to the confirm step for that entry.
- [ ] The picker opens both from the ⋮ menu and as task-002's correlation fallback, via one path.
- [ ] The ⋮ menu item is gated by the same predicate as the row affordance.
- [ ] All user-visible strings match § 12's copy deck verbatim.
