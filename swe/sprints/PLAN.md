...
| task-006 | Live E2E proof of the whole size contract (fresh open, long-command editing, drag, splits, reattach, reconnect, two clients, large-output replay) + docs sync incl. three wrong statements in `packages/server/AGENTS.md` and a stale renderer comment — **closed 2026-08-21 with the live sequence waived by the user**; docs + gates delivered, the browser acceptance boxes in `terminals.md` stay unticked as unproven, not pending | test + docs | task-001, task-002, task-003, task-004, task-005 | AGENTS.md (server, web-client); packages/web-client (features/terminal/TerminalPanel); features/terminals, feature-panels-ui |
...
| task-006 | Live E2E proof of the three strands **and their combinations** (font change → new width → restore laid out for it; exited terminal not restored on reconnect; disabled-daemon fallback) + sprint-052 regression sweep + docs sync — **closed 2026-08-21 with the live sequence waived by the user** (same treatment as sprint-052/task-006); docs sync + full gate delivered, five stale doc claims fixed, the live-only acceptance boxes stay unticked as unproven, not pending | test + docs | task-001, task-002, task-003, task-004, task-005 | AGENTS.md (protocol, client, server, web-client); features/terminals, feature-panels-ui; architecture/design-system |
| task-007 | Broadcast PTY size on resize (ends multi-client belief drift, the limitation sprint-052 documented rather than fixed) + degrade the snapshot ring to the naive cut instead of dropping everything on an unterminated escape sequence — **done 2026-08-21**; unit-tested at the RPC layer and via a pure client-side helper, the two-browser live sequence waived by the user alongside task-006's | bugfix | none (coordinated with task-003 — `TerminalsUpdateMessage` extended, not duplicated; the two effects stay separate since exit-watch is tab-store state and this is a component-local ref) | packages/server (terminal/{terminal-manager,terminal-rpc,+tests}); packages/web-client (features/terminal/{TerminalPanel,terminal-size,+tests}, hooks/use-terminal-exit-watch); features/terminals |
...
### sprint-063-html-file-preview
> **Where this comes from.** `features/html-file-preview.md`, written for this sprint pair. An
> `.html` file opens as source today; agents produce HTML constantly (coverage reports, benchmark
> dashboards, plots, exported diagrams) and reading one means leaving the app — impossible when the
> daemon is a remote box reached over the relay, since the file is not on the browser's machine.
>
> **The design is grounded in measurement, not assumption.** Probed in headless Chromium on
> 2026-08-19 (parent on a real `https` origin, child `<iframe sandbox="allow-scripts">`, no
> `allow-same-origin`): child `location.origin` is `"null"`; `parent.document` and `localStorage`
> both throw `SecurityError`; a parent-created `blob:` URL **cannot** be fetched or `<img>`-loaded
> from inside the sandbox (`Failed to fetch`); `data:` subresources (img/script/stylesheet) all load;
> `srcdoc`'s `document.baseURI` is **the parent document's URL**; and an injected
> `<meta http-equiv="Content-Security-Policy">` blocks remote img/script/fetch while leaving inline
> and `data:` content working. Those five facts decide everything: `srcdoc` + `allow-scripts` for the
> document, `data:` (never object URLs) for assets in sprint-064, an injected `.invalid` `<base>` so
> an un-inlined relative ref fails cleanly instead of being answered by the SPA's history fallback,
> and a meta CSP as the network lever.
>
> **Isolation is not a nicety here.** The app's origin holds the daemon password and connection state
> in `localStorage` (`providers/kv-store.ts`) plus a live authenticated WebSocket, so a previewed
> document running in the app's origin would be a credential compromise. `allow-same-origin` is
> therefore banned outright — with `allow-scripts` it re-grants exactly that — and the token list
> lives in one frozen module constant with a guard test, because "just add `allow-same-origin` so the
> stylesheet loads" is precisely how this regresses later.
>
> **Remote loading is allowed by default — recorded product decision.** The common case is an
> agent-produced report pulling Plotly/Chart.js/Tailwind from a CDN, and friction there is worse than
> the residual risk, which the sandbox already bounds to "the document can reach the network", never
> to app state. Blocking is a per-tab toggle, not the default.
>
> **Two structural fixes ride along, because HTML is the first new viewer since molecule support and
> it hit both.** (a) The registry promises "one entry" and costs four edits across two directories;
> the fourth, `LIVE_REFRESH_KINDS` (`hooks/use-file-live-refresh.ts:23`), is defaulted silently when
> forgotten despite its own comment claiming an explicit choice is required — so registration
> collapses into one descriptor table with derived lookups and a required `liveRefresh` field.
> (b) `ViewerProps` is `{ path }` only, so no viewer can resolve a sibling file; that is already a
> live defect (`MarkdownFileViewer.tsx:61` renders `<Markdown>` with no `assetBase`, so relative
> images in a previewed `.md` never resolve) and would equally block HTML, so the contract gains
> `workspaceCwd` once, for both.
>
> **What is deliberately NOT done.** Molecule's separate tab-kind dispatch (`open-file-tab.ts` →
> `PANEL_BY_KIND.molecule`) stays as-is: folding it into the registry touches persisted tab
> identities (`reopen-client-tabs.ts`) and is unrelated risk. HTML goes through the registry, so it
> inherits `FilePanel`'s header, File/Diff toggle and live refresh for free — a third dispatch
> mechanism is exactly what this sprint exists to avoid. `svg` stays `image` (an `<img>`-rendered SVG
> executes no scripts; routing it here would be a security regression dressed as a feature). No
> protocol, server, persistence or test-infrastructure work; pure logic is unit-tested in the node
> environment and the component is verified in a real browser, per project convention.

| Task | Title | Type | Depends on | Covers |
|------|-------|------|------------|--------|
| task-001 | Descriptor-driven viewer registry (`VIEWER_REGISTRY` with lazy component + extensions/mimePrefixes + **required** `liveRefresh`; `VIEWER_BY_KIND`, extension/MIME lookup and the live-refresh set all derived at module load) with `detectViewerKind`/`isMoleculeFile` outputs and precedence frozen (no new matching steps — basename matching stays molecule-only in `isMoleculeFile`) — `timeline/image-src.ts` uses `detectViewerKind` as its "is this an image?" gate, so the existing test expectations must pass unmodified; plus `ViewerProps.workspaceCwd` threaded from `FilePanel` and forwarded by `MarkdownFileViewer` to `<Markdown>`, fixing relative images in previewed markdown; new tests: no extension claimed twice, live-refresh set asserted by inclusion+exclusion (not exact equality, so task-003's `html` extends it without edits) | refactor | none | packages/web-client (viewer-registry + test, use-file-live-refresh, FilePanel, all five viewers); features/html-file-preview § Viewer descriptor registry, § Viewer props |
| task-002 | Shared file-source size ladder: `hooks/use-file-source.ts` composing `useFileRead` → `FileTooLargeError` → `useFileText` → terminal download-only state, keeping the existing pure tier selector as its decision core; `TextViewer` re-expressed on it with zero behavior change, `MarkdownFileViewer` switched onto it so a >5 MiB markdown file streams instead of rendering the string `file_too_large`. Needed by HTML specifically: coverage/benchmark reports routinely exceed the daemon's 5 MiB inline-read cap | refactor | none | packages/web-client (use-file-source, TextViewer, text-viewer-state + test, MarkdownFileViewer); features/html-file-preview § Source loading |
| task-003 | `html-sandbox.ts` (frozen `HTML_SANDBOX_TOKENS` = `["allow-scripts"]`, `HTML_PREVIEW_BLOCKING_CSP`, `PREVIEW_BASE_HREF`, `assembleHtmlPreview(source, { assets, blockRemote })` doing meta-CSP injection, `<base>` injection only when the source declares none, and an asset-substitution seam left inert for sprint-064) + guard test asserting `allow-same-origin`/`allow-top-navigation*`/`allow-popups` never appear; a measured fragment-anchor decision (in-page `#` links must scroll, not navigate the frame to the `.invalid` host); `HtmlViewer` rendering `<iframe sandbox srcDoc referrerPolicy="no-referrer" allow="">` with Preview/Source (`CodeView`), "Block remote resources" and Reload, `srcdoc` memoized so unrelated re-renders never reload the document; one registry descriptor (`html`/`htm`/`xhtml`, `liveRefresh: true`) | feature | task-001, task-002 | packages/web-client (html-sandbox + test, HtmlViewer + CSS, viewer-registry); features/html-file-preview § Preview document assembly, § Network policy, § Base-URL neutralization |
| task-004 | Docs sync + pre-ship verification: `packages/web-client/AGENTS.md` gains source-layout entries and two invariants — "HTML preview sandbox" (never `allow-same-origin` and why, no object URLs inside the sandbox, `srcdoc`'s base URL is the app's, CSP-as-network-policy with remote allowed by default) and "Adding a file viewer" (one descriptor entry, explicit `liveRefresh`, a new tab kind needs justification); `features/feature-panels-ui.md`'s file-preview section resynced to the real viewer set; then full gates from clean plus a browser matrix (three theme variants, split pane, markdown-with-relative-image, image/video/binary sanity, sandbox probe) recorded as observations | docs + test | task-001, task-002, task-003 | packages/web-client/AGENTS.md; features/feature-panels-ui, features/html-file-preview |

### sprint-064-html-preview-local-assets
> **What sprint-063 cannot reach.** The document renders; its neighbours do not. `./style.css`,
> `./app.js` and `./img/logo.png` resolve against sprint-063's injected `.invalid` base and fail, so
> a multi-file report previews unstyled — correct and safe, but not useful for the exact artifacts
> this feature exists to read.
>
> **`data:` is forced, not chosen.** The obvious fix — download each asset, `URL.createObjectURL`,
> rewrite the refs — cannot work: a sandboxed opaque-origin document cannot fetch a parent-created
> `blob:` URL (measured 2026-08-19, `Failed to fetch` / `<img>` error). `data:` subresources do load,
> including scripts and stylesheets, so inlining is the only mechanism available. MIME must be derived
> **client-side** from the extension: the daemon's `mimeHintForFile` (`files/file-explorer.ts:467-478`)
> has no `.html`/`.css`/`.js` entries and answers `application/octet-stream`, which browsers reject
> for stylesheets and classic scripts — and a newer client must work against an older daemon anyway.
>
> **Confinement is the security-relevant half.** With remote loading on by default, a document that
> names `../../../.ssh/id_rsa` and is fetched on its behalf could read the bytes back out of its own
> inlined `data:` URI and post them anywhere. So a candidate is **percent-decoded first** (once,
> non-throwing — encoded separators become real ones *before* any normalization), resolved against
> the document's directory, normalized with `collapseDotSegments`, rejected if any `.`/`..` segment
> survives, and then required under the confinement root by a segment-aware check (never a bare
> string prefix, which would accept a `/ws-evil` sibling of `/ws`). The root is the tab's workspace
> root — narrowed to the document's own directory when that root is the home directory itself
> (workspace-less tabs fall back to `cwd = "~"`, which would otherwise put `~/.ssh` *inside* the
> root). The decode-first order is load-bearing: normalize-then-decode lets
> `foo%2F..%2F..%2F..%2Fetc%2Fpasswd` pass as one opaque segment and decode back into a traversal.
> This gate is pure and exhaustively tested, including the percent-encoded spellings of both `..`
> and the separator.
>
> **Rewriting is best-effort presentation; the sandbox is the boundary.** That is what makes
> string-level attribute rewriting (rather than a DOM parse, unavailable in this package's node-only
> test environment) a legitimate choice: a missed ref degrades to exactly sprint-063's outcome. Caps
> (64 refs, 2 MiB each, 16 MiB total) plus a muted "N references not inlined" note keep a skip
> visible rather than silent.
>
> **Known limitations, recorded rather than papered over:** `@import` chains beyond one nested level,
> refs inside the document's own inline `<style>` blocks, and nested `<iframe src>` are not resolved;
> only the document is watched, so an edited stylesheet updates on Reload (per-asset watches would
> need N ref-counted subscriptions per preview). No daemon, protocol or persistence work.

| Task | Title | Type | Depends on | Covers |
|------|-------|------|------------|--------|
| task-001 | Pure `html-assets.ts`: `extractLocalAssetRefs` over the scanned attribute contexts (`link[rel=stylesheet][href]`, `script[src]`, `img[src]`/`img[srcset]`, `source[src]`/`source[srcset]`, `video[src]`/`video[poster]`, `audio[src]`), `confinementRoot` (home-narrowing rule) + `confineAssetRef` (percent-decode first → resolve against the doc dir → `collapseDotSegments` → reject surviving dot segments → segment-aware root check; reuses `href-resolution.ts`'s scheme detection and non-throwing decode but **not** its normalize-before-decode order, which is exploitable as a gate), `ASSET_LIMITS`, `mimeForAssetPath`, `dataUri`, `rewriteCssUrls`; plus `assembleHtmlPreview`'s substitution made real (attribute values only, never document text). Exhaustive node tests: quoting/case/attribute-order variants, `srcset` descriptors + a `data:`-URI substitution round-trip, dedupe, every skip reason, `..`/`%2e%2e`/`%2F`-separator traversal rejected, sibling-prefix root rejected, home-narrowing, entities matched as authored, cap arithmetic | feature | s063/task-003 | packages/web-client (html-assets + test, html-sandbox); features/html-file-preview § Local asset inlining |
| task-002 | Asset fetch orchestration + inlining: a framework-free loader over an injected byte-fetch (bounded parallelism, per-asset and total caps enforced as bytes arrive, one fetch per de-duplicated path, one nested `url()` pass per inlined stylesheet, per-ref failure isolation) driven from `HtmlViewer` by a query keyed on (path, memoized source hash) so assets refetch with the document and not on a toggle; transport is `transferFor(daemon).download` directly — `useFileDownload`'s per-path revoke-on-unmount object-URL ownership is the wrong shape, the same reason `use-inline-image` reached past it; single-load rule: with zero confined refs render immediately, otherwise hold until the bundle settles (an assetless intermediate `srcdoc` would reload the document and run its scripts twice) — individual failed/over-cap refs settle as skipped, never blocking the preview; skipped refs surface as a muted aggregated note | feature | task-001 | packages/web-client (html-asset-loader + test, HtmlViewer, rpc-keys); features/html-file-preview § Local asset inlining, § Error handling |
| task-003 | Fixture verification + docs sync: a multi-file fixture (`report.html` + `style.css` with a `url()` background + `app.js` + `img/logo.png`), a CDN variant, and a hostile variant naming `../../../.ssh/id_rsa`, `/etc/passwd` and the `%2F`-encoded spelling — recorded evidence that assets are `data:` (no `blob:` anywhere), that out-of-workspace refs produce **zero** download RPCs (daemon debug log as an independent witness beside the loader's counting fake), that inlined `data:` assets (stylesheet **and** script) still work under the blocking CSP (resolving the spec's per-directive `data:` TODO(verify)), that a home-rooted tab inlines same-directory assets only, and that the sandbox probe still fails to reach `parent.document`/`localStorage`; then AGENTS.md/spec/feature-panels-ui updates and full gates from clean | docs + test | task-001, task-002 | packages/web-client/AGENTS.md; features/html-file-preview, features/feature-panels-ui |

### sprint-065-provider-auth-ui
> **Prerequisite, stated plainly.** Sprint-055 is now **done** (protocol schemas, `PiAuthRuntime`
> seam, `ProviderAuthService`, RPC handlers + bootstrap wiring, and a live E2E run against a real
> daemon + real Pi all shipped and verified — see its `done/` task summaries). Every task here
> consumes that wire contract, so 055 had to run first; the numbering already encodes it. This
> sprint (the browser half) is now startable.
>
> **What it closes.** Sprint-054 gave `pi-studio auth login` to anyone with a shell on the daemon
> host. Sprint-055 exposes the same Pi engine over five RPC pairs. Neither helps the web-client user
> — especially over the relay to a headless box — who still has no way to configure a model provider,
> and whose first signal that nothing is configured is a failed turn. This sprint is the surface that
> makes the previous two reachable from a browser.
>
> **The SDK is an adapter, and it needs no new plumbing.** `DaemonClient` already exposes exactly the
> three seams required — `request(type, params, timeoutMs?)`, `onSessionMessage(handler)`,
> `hasFeature(flag)` — so the four facade methods (`listProviderAuth`, `loginProvider`,
> `logoutProvider`, `hasProviderAuthCapability`) are pure correlation logic on top of what exists,
> following the flat `listExtensionPacks`/`setExtensionPacks` precedent rather than a new handle class.
>
> **One ordering bug is designed out up front.** The daemon starts Pi's flow the moment it handles
> `provider_auth_login_request`, so an `auth_url` or `prompt` event can arrive *before* the login
> response carrying `flowId` is processed. The SDK therefore subscribes **first** and buffers until
> the flowId is known. Written the natural way — await the response, then subscribe — the first prompt
> is silently dropped and the dialog waits forever. It gets an explicit regression test.
>
> **Three questions the scope left open, settled by reading the code.** (a) There is **no settings
> screen and no router** in web-client: `routes/WorkspacePage.tsx` is one shell, and the only
> settings-like state (`theme/appearance-store.ts`) has no panel of its own, while
> `app-navigation-screens.md`'s `/settings/hosts/[serverId]/providers` is spec-only. So this sprint
> ships the **settings dialog shell itself** (user direction, 2026-08-20): a gear at the
> ConnectionBar's top-right opens a large `Dialog`-primitive modal with a category sidebar, Model
> Providers as the sole category — barely more work than a bare modal (no router, one lazy chunk),
> and it *is* the settings IA rather than something that migrates into one later; the gear is
> hidden without the `providerAuth` capability while every registered category is capability-gated,
> becoming unconditional once a capability-independent category (Appearance) ships. Building any
> further category here would be a second sprint wearing this one's name.
> (b) There is **no QR component** to reuse: `qrcode` appears only daemon/CLI-side for
> terminal rendering, so a small browser component wrapping `toDataURL` is new, with `qrcode` added as
> a **devDependency** because web-client ships no runtime deps — only its prebuilt `dist` — and Vite
> bundles it into the lazy chunk. (c) The nudge goes in `Timeline.tsx`'s existing empty-state branch
> using the `EmptyState` primitive; there is no app-wide banner pattern, and inventing one for this
> would be the largest visual change in a sprint that otherwise adds no chrome.
>
> **Why the dialog is two tasks.** Task-004 ships prompt inputs, the status region and terminal
> states — which is a complete, useful API-key login on its own. Task-005 then adds the OAuth
> presentation surface, whose defining constraint is concurrency: over the relay any localhost
> callback server binds on the *daemon* host and never sees the remote user's redirect, so Pi races
> `auth_url` against a `manual_code` prompt and cancels the prompt via `prompt_cancelled` if the
> callback wins. Both must be on screen together, which is exactly why task-002's reducer keeps the
> url and the prompt in state simultaneously.
>
> **Nothing is persisted client-side** — no secret, no flow state, nothing in `localStorage`; the only
> durable artifact is the daemon host's `auth.json`, owned by Pi. Flows die with the socket and are
> never resumed (a retry is cheap). Out of scope by decision: the settings IA itself, CLI `--host`
> remote login, and any credential display or editing.

| Task | Title | Type | Depends on | Covers |
|------|-------|------|------------|--------|
| task-001 | SDK: four `PiStudioClient` methods + local flow-event types and type guard (passthrough push family, no protocol schema) + flow correlation — **subscribe-before-request buffering**, prompt→`respond` bridging, `prompt_cancelled` rejecting the pending prompt, `opts.signal`→cancel, one active flow per client, disconnect settling `connection_lost`, stale-flow events dropped | feature | s055/t001-005 | packages/client (pistudio-client, index, pistudio-client.test — existing `makeScriptedDaemon()` harness); features/provider-auth-ui § Public Contract |
| task-002 | Pure `login-flow.ts` reducer: exhaustive over every event kind, `auth_url`-after-`prompt` keeping both live, `info` appends vs `progress` replaces, `prompt_cancelled` id-matched, `done` terminal; no callbacks or timers in state (the prompt resolver stays at the component boundary, which is what keeps this testable) | feature | task-001 | packages/web-client (features/provider-auth/login-flow + test, node-env, no jsdom); features/provider-auth-ui § Behavior & Algorithms |
| task-003 | **Settings dialog shell** (gear at ConnectionBar top-right → `Dialog`-primitive modal, category sidebar + content pane, local category registry) + **Model Providers** as the sole category: capability-gated (gear absent ⇒ zero RPCs), badges for stored key / oauth / `env: VAR` / unconfigured plus a distinct `"unknown"` state (a bounded-out `checkAuth` must never invite a re-login over a working credential), subscription tag + `oauthLoginLabel`, logout behind a confirm with the running-agent caution and `stillConfigured` reporting, shared query key, lazy-mounted | feature | task-001 | packages/web-client (features/settings/SettingsDialog + module.css, features/provider-auth/ModelProvidersPanel + module.css, features/connection/ConnectionBar, lib/connection/rpc-keys); features/provider-auth-ui § Web UI surface |
| task-004 | Login dialog core — masked `secret`, `text`/`manual_code`, `select` with descriptions, status region off reducer state, `done ok` → list invalidation + auto-close, `done !ok` → error + `Try again` starting a **fresh** flow, Cancel/Esc → abort → server-side cancel, disconnect rendered as error not hang. **API-key login works end-to-end after this task** | feature | task-001, task-002, task-003 | packages/web-client (features/provider-auth/LoginDialog + module.css); features/provider-auth-ui § Web UI surface |
| task-005 | OAuth presentation: new `QrCode` component over `qrcode`'s browser `toDataURL` (+ devDependency, lazy-chunk only), `auth_url` link + copy + QR + instructions, `device_code` with verification link and view-local countdown, all rendered **concurrently** with a live `manual_code` prompt; `prompt_cancelled` removes only the input | feature | task-004 | packages/web-client (features/provider-auth/QrCode + module.css, LoginDialog, package.json); features/provider-auth-ui § Web UI surface |
...
> oversight. No retained surfaces (a later sprint — 070 was taken by the thinking-level selector).
> And no destructive `confirm` variant, because nothing on the
...
### sprint-070-thinking-level-selector
> **What it ships.** A thinking-level selector in the composer (the "Thinking | brain-icon badge →
> levels combobox" row `features/composer-ui.md`'s controls table has specified since sprint-015),
> with the level persisted per session — surviving page reloads, daemon restarts, and session
> resumes — and converging across every connected client. Scope: the new
> `features/thinking-level-selector.md`, written from **verified** Pi RPC behavior (dist source +
> live checks), not docs alone.
>
> **The bug it fixes on the way in.** `AgentSession.setThinkingOption?` is declared
> (`provider-contract.ts:227`) and called via `?.` (`session-operations.ts:163`) but implemented by
> **nobody** — `update_agent_request` with `thinkingOptionId`, and therefore
> `pi-studio agent update --thinking`, persists config and silently applies nothing. Task-001 makes
> it real; task-003 routes the update path through the same helper as the new dedicated RPC so the
> two write paths cannot diverge.
>
> **Three verified findings drive the design.** (a) `set_model`'s response is the bare Model
> object — NO `thinkingLevel` (only `cycle_model`'s response carries one) — yet a model switch is
> exactly when Pi clamps the level; the Pi adapter therefore re-reads `get_state` after every
> switch, and the daemon persists + broadcasts the **effective** level (clamp write-back), never
> the requested one. Without it: pin `max`, switch to a non-reasoning model → record says `max`,
> truth is `off`, dead-session `list_agents` lies. (b) `get_available_models` returns raw full
> Model objects (`modelRuntime.getAvailableSnapshot()`) including `reasoning` +
> `thinkingLevelMap`, which the daemon's `listModels` mapping currently discards — so draft-time
> level discovery is a lookup in the model list the web client **already fetches and caches**
> (zero new spawns; the earlier transient-spawn+cache design was reviewed out as unnecessary). The
> 11-line derivation mirrors pi-ai's `getSupportedThinkingLevels` (not importable: nested
> transitive dep) and any drift self-corrects at set time because Pi clamps authoritatively.
> (c) Replay order is model THEN thinking — Pi clamps thinking against the model, so
> thinking-first is overwritten; `spawnOrResumeSession` encodes the order, and skips the replay
> entirely when `config.thinkingOptionId` is unset rather than clobbering Pi's own
> JSONL-restored level.

| Task | Title | Type | Depends on | Covers |
|------|-------|------|------------|--------|
| task-001 | Pi provider: `set_thinking_level`/`get_available_thinking_levels`/`get_state.thinkingLevel` wired into `setThinkingOption`/`listThinkingLevels`/`discoverState`/`getRuntimeInfo`; **`get_state` re-read after `setProviderModel`** (set_model's response lacks the level); `listModels` carries per-model `reasoning` + derived `thinkingLevels`; mock provider static equivalents | feature | none | packages/server (agent/providers/pi, mock, provider-contract); features/thinking-level-selector § Ground truth |
| task-002 | Protocol schemas + `agent_set_thinking_request/response` (draft-pin branch mirroring `handleSetModel`'s, effective-level response) + `agent_thinking_levels_request/response` (live sessions) + `thinkingLevels` feature flag + `persistThinking`; registered in bootstrap AND dev-bootstrap | feature | task-001 | packages/protocol; packages/server (slash-command-operations, daemon bootstraps); features/thinking-level-selector § Public contract |
| task-003 | Persistence chain: replay in `spawnOrResumeSession` (after model, skipped when unset), clamp write-back + broadcast on `handleSetModel`/`handleCycleModel`, `handleUpdate`'s thinking branch routed through the same helper, `list_agents` projects `thinkingLevel` (live wins over pinned), `resolve_default_model` returns the default level its `--no-session get_state` already receives | feature | task-002 | packages/server (agent-service, session-operations, slash-command-operations, bootstraps); features/thinking-level-selector § Persistence chain |
| task-004 | Client SDK: `agent(id).setThinking(level)` / `.listThinkingLevels()` typed facades; `ProviderModel.reasoning/thinkingLevels` + `ResolveDefaultModelResponse.thinkingLevel` typing; scripted-daemon fixtures | feature | task-002, task-003 | packages/client (pistudio-client, test-support); features/thinking-level-selector § Public contract |
...
(s055, done), browser (s065, not yet implemented).
`features/thinking-level-selector.md` (new with sprint-070) is fully planned by that sprint's six
tasks — provider layer, wire surface, persistence chain, SDK, web UI, live E2E close — and also
covers the previously-unowned `composer-ui.md` § toolbar-controls "Thinking" row, plus the dormant
`thinkingOptionId` plumbing `agent-providers.md` lists as an optional session capability
(`setThinkingOption` — declared since sprint-005, implemented by nobody until now).
...
live-verified against a real dev daemon + browser); the **retained surfaces** (widget
blocks, status strip, title suffix — § 09, § 10) are scoped by the same document but **not yet
planned** (the sprint number this passage once forecast for them, 070, was taken by the
thinking-level selector), so this row claims no coverage for them. The split
...
diverge.
`features/html-file-preview.md` is planned across two sprints: sprint-063 (the registry
generalization, the shared size ladder, and the sandboxed document preview) and sprint-064 (local
asset inlining). Sprint-063 stands alone as a complete, useful feature for self-contained HTML — the
common agent-produced case — and sprint-064 is what makes a multi-file report render as authored;
splitting them keeps the security-relevant asset-fetch gate in a sprint of its own rather than
bundled behind a UI change. The spec's browser-platform-constraints section is measured behavior
(headless Chromium, 2026-08-19), not inference, and is the grounding both sprints' decisions cite.
...
| features/html-file-preview.md | s063/t001-004 (descriptor registry + `ViewerProps.workspaceCwd`, shared size ladder, `html-sandbox`+`HtmlViewer`, docs/verification); s064/t001-003 (pure extraction/confinement/rewriting, fetch+`data:` inlining, fixture verification) |
