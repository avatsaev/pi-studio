 ...
 * `startRelayServer`, logs the package version + bound address + health endpoint, and shuts down
 * cleanly on `SIGINT`/`SIGTERM`. This is what `pi-studio relay start` (packages/cli) spawns as a
 * detached child process, and what `npx @av-pi-studio/relay` runs directly for a
 * foreground/manual deploy.
 *
...
async function main(): Promise<void> {
  ...
  log.info({ version }, `pi-studio relay v${version} starting`);
