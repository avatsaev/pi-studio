...
import { normalizeDaemonUrl } from "@pi-studio-ui/lib/connection/normalize-url.js";
...
export function ServerDetail({ server }: { server: SavedServer }) {
  ...
  const connectedUrl = useConnectionStore((s) => s.connectedUrl);
  ...
  // connectToServer / the toolbar); the settled connection reports connectedUrl.
  const pendingHost = useUiStore((s) => s.host);
  const normalizedUrl = normalizeDaemonUrl(server.url);
  const isLive = isConnectedToDaemon(status, connectedUrl, server.url);
  const isConnecting = status === "connecting" && normalizedUrl === normalizeDaemonUrl(pendingHost);
  ...
  return (
    <>
      ...
      {connectionError && (isLive || normalizedUrl === normalizeDaemonUrl(pendingHost)) && (
